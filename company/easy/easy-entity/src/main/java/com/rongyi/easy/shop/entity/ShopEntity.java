...
//    private List<ObjectId> exclusive_category_ids;//店铺所属专属分类
	...
	public ShopEntity(ShopParam param) throws Exception {
		...
		this.candidateShop = param.getCandidateShop();
		this.newShop = param.getNewShop();
		this.featuredShop = param.getFeaturedShop();
		if(StringUtils.isNotBlank(param.getName())){
			this.acronym = Pinyin4jUtil.getShortPinYin(param.getName());
			this.slug = hanyuToPinyin(param.getName());
		}
		...
		this.location = param.getLocation();//店铺坐标
	}
