...
//		logger.info(">>>"+result);
	...
	public static String httpGETGzip(String url) {
		URL u = null;
		...
		try {
			...
			logger.info("ContentEncoding->" + con.getContentEncoding());
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			br = new BufferedReader(new InputStreamReader(new GZIPInputStream(con.getInputStream()), "UTF-8"));
			String temp;
...
//        json方式
		...
		try {
			...
			logger.info("httpPostWithJSON|StatusCode():" + resp.getStatusLine().getStatusCode());
			if(resp.getStatusLine().getStatusCode() == 200) {
	            HttpEntity he = resp.getEntity();
	            logger.info("httpPostWithJSON|he:" + resp.getEntity());
	 ...
	 * @return
	 */
	public static String httpPostWithJsonString(String url, String json) {
		logger.info("httpPostWithJsonString|url:" + url + ";json:" + json);
        HttpPost httpPost = new HttpPost(url);
        CloseableHttpClient client = HttpClients.createDefault();
        String respContent = null;
