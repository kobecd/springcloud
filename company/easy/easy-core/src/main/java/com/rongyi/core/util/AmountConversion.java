...
public abstract class AmountConversion {
     ...
     * @return 元单位金额
     */
    public static BigDecimal getYuan(Integer fen) {
        BigDecimal val = BigDecimal.ZERO;
        if (fen != null && fen > 0) {
            val = BigDecimal.valueOf(fen).divide(new BigDecimal(100), 2, BigDecimal.ROUND_HALF_UP);
        }
        return val;
    }
     ...
     * @return 元单位金额
     */
    public static int getFen(BigDecimal yuan) {
        int val = 0;
        if (yuan != null && yuan.compareTo(BigDecimal.ZERO) > 0) {
            val = yuan.multiply(BigDecimal.valueOf(100.00D)).setScale(2).intValue();
        }
        return val;
    }
